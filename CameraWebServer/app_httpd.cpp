...
//New handler for unlock
static esp_err_t unlock_handler(httpd_req_t *req) {
  Serial.println("Received unlock request");
    digitalWrite(Relay, HIGH);
    delay(10000);
    digitalWrite(Relay, LOW);
    Serial.println("Unlock sequence completed");
  httpd_resp_send(req, "Unlock endpoint reached", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &unlock_uri); //Register new URI
  }
